...
def train_and_evaluate_ridge_regression():
    ...
    grid_search = GridSearchCV(estimator=ridge_model, param_grid=param_grid, cv=5, scoring='neg_mean_squared_error', n_jobs=-1, verbose=1)
    ...
    y_pred_test_final = best_ridge_model.predict(X_test_final)
    ...
    output_csv_path = os.path.join(BASE_DIR, 'results/ridge_regression_predictions.csv')

    # Save the merged DataFrame to a CSV file
    test_df_to_csv.to_csv(output_csv_path, index=False)

    eval_df = pd.read_csv(os.path.join(BASE_DIR, 'results/evaluation.csv'))
    new_row = {'algorithm': 'Ridge Regression Model with Hyperparameter Tuning', 'train Mean Squared Error': mse_train, 'train Mean Absolute Error': mae_train, 'test Mean Squared Error': mse_test_final, 'test Mean Absolute Error': mae_test_final}
