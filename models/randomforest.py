...
y_pred_test_final = best_rf_model.predict(X_test_final)
...
output_csv_path = os.path.join(BASE_DIR, 'results/random_forest_regression_predictions.csv')

# Save the merged DataFrame to a CSV file
test_df_to_csv.to_csv(output_csv_path, index=False)

eval_df = pd.read_csv(os.path.join(BASE_DIR, 'results/evaluation.csv'))
new_row = {'algorithm': 'Random Forest Regression Model with Hyperparameter Tuning', 'train Mean Squared Error': mse_train, 'train Mean Absolute Error': mae_train, 'test Mean Squared Error': mse_test_final, 'test Mean Absolute Error': mae_test_final}
