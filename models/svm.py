...
def train_and_evaluate_svm_regression():
    print("\nSVM Regression Model with Hyperparameter Tuning")
    # Load the dataset
    ...
    y_pred_test_final = best_svm_model.predict(X_test_final)
    ...
    output_csv_path = os.path.join(BASE_DIR, 'results/svm_regression_predictions.csv')

    # Save the merged DataFrame to a CSV file
    test_df_to_csv.to_csv(output_csv_path, index=False)

    eval_df = pd.read_csv(os.path.join(BASE_DIR, 'results/evaluation.csv'))
    new_row = {'algorithm': 'SVM Regression Model with Hyperparameter Tuning', 'train Mean Squared Error': mse_train, 'train Mean Absolute Error': mae_train, 'test Mean Squared Error': mse_test_final, 'test Mean Absolute Error': mae_test_final}
    eval_df.loc[len(eval_df)] = new_row
    eval_df.reset_index()
    eval_df.to_csv(os.path.join(BASE_DIR, 'results/evaluation.csv'), index=False)

train_and_evaluate_svm_regression()
